...
public class HeaderEntity {
	 ...
	 * Whether this entity has attributes which specifies it's value.
	 */
	//Dan added this to support "=" option to specify parameter value in header file
	private boolean hasSpecificParamVal;
	...
	private void parseHeaderStr(String headerStr) throws Exception {
		...
		//Dan added this to support specifying parameter value to use in the header file
		if(headerStr.indexOf(":")>-1) {
			readData = false;
			paramVal = headerStr.substring(headerStr.indexOf(":")+1);
			hasSpecificParamVal=true;
		...
		if(headerStr.startsWith("+")) {
			...
			readData = true; 
			hasSpecificParamVal=false; //overrides provided val if + exists
			headerStr = headerStr.substring(1);
		} 
	 ...
	 * false otherwise.
	 */
	public boolean doesSpecifyParamValue() {
		return hasSpecificParamVal;
	}
	...
	public Node doCreate(Document doc, String currTableVal, Element merge) {
		...
		if(hasSpecificParamVal) {
			ret.setTextContent(paramVal);
		}
		return ret;
	}
